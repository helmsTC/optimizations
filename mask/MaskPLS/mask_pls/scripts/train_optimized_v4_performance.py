"""
Performance-optimized training script for MaskPLS that maintains accuracy
This version fixes the performance issues while keeping good training speed
"""
...
class OptimizedVoxelizer:
    """High-resolution voxelizer for better accuracy"""
    def __init__(self, spatial_shape, bounds, device='cuda'):
        # Use higher resolution for better accuracy (closer to original)
        self.spatial_shape = (128, 128, 64)  # Much higher resolution
        self.bounds = bounds
        self.device = device
        ...
        self.cache_size = 100  # Smaller cache for memory
        
    def voxelize_batch_optimized(self, points_batch, features_batch):
        """Optimized batch voxelization with high resolution"""
        B = len(points_batch)
        D, H, W = self.spatial_shape
        C = features_batch[0].shape[1] if len(features_batch) > 0 else 4
        
        # Use sparse representation for efficiency at high resolution
        voxel_grids = torch.zeros(B, C, D, H, W, device=self.device, dtype=torch.float16)
        voxel_counts = torch.zeros(B, 1, D, H, W, device=self.device, dtype=torch.float16)
        normalized_coords = []
        valid_indices = []
        ...
        for b in range(B):
            ...
            # Accumulate features efficiently
            for c in range(C):
                voxel_grids[b, c].view(-1).scatter_add_(0, flat_indices, valid_feat[:, c].half())
            
            # Count accumulation
            voxel_counts[b, 0].view(-1).scatter_add_(0, flat_indices, torch.ones_like(valid_feat[:, 0]).half())
        ...
        voxel_grids = voxel_grids.float()  # Convert back to float32
        
        return voxel_grids, normalized_coords, valid_indices


class PerformanceMaskLoss(torch.nn.Module):
    """Mask loss matching original implementation closely"""
    def __init__(self, cfg, data_cfg):
        ...
        # Match original weights exactly
    ...
    def forward(self, outputs, targets, masks_ids, coors):
        """Loss computation matching original implementation"""
        losses = {}
        ...
        # Apply weights exactly as original
        weighted_losses = {}
        for l in losses:
    ...
    def loss_classes(self, outputs, targets, indices):
        """Classification loss matching original"""
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, masks_ids):
        """Mask losses matching original implementation exactly"""
        ...
        # Sample points exactly as original
        with torch.no_grad():
        ...
        # Use JIT-compiled loss functions for consistency
        losses = {
    ...
    def dice_loss(self, inputs: torch.Tensor, targets: torch.Tensor, num_masks: float):
        """Dice loss exactly as original"""
        inputs = inputs.sigmoid()
        numerator = 2 * (inputs * targets).sum(-1)
        denominator = inputs.sum(-1) + targets.sum(-1)
        loss = 1 - (numerator + 1) / (denominator + 1)  # +1 as in original
        return loss.sum() / num_masks
    
    @torch.jit.script_method
    def sigmoid_ce_loss(self, inputs: torch.Tensor, targets: torch.Tensor, num_masks: float):
        """BCE loss exactly as original"""
    ...
    def _get_tgt_permutation_idx(self, indices, n_masks):
        """Target permutation matching original"""
...
class PerformanceOptimizedMaskPLS(LightningModule):
    """Performance-optimized MaskPLS with accuracy fixes"""
    def __init__(self, cfg, onnx_interval=5):
        ...
        # Use better channel progression for features
        cfg.BACKBONE.CHANNELS = [64, 128, 256, 256, 256]  # Match original better
        
        # Create model
        self.model = MaskPLSSimplifiedONNX(cfg)
        
        # Use performance-optimized loss
        self.mask_loss = PerformanceMaskLoss(cfg.LOSS, cfg[dataset])
        self.sem_loss = SemLoss(cfg.LOSS.SEM.WEIGHTS)
        
        # High-resolution voxelizer
        self.voxelizer = OptimizedVoxelizer(
            (128, 128, 64),  # Higher resolution
            cfg[dataset].SPACE,
            device='cuda'
        )
    ...
    def forward(self, batch):
        """Forward pass with high-resolution voxelization"""
        points = batch['pt_coord']
        features = batch['feats']
        
        # High-resolution voxelization
        voxel_grids, norm_coords, valid_indices = self.voxelizer.voxelize_batch_optimized(points, features)
        ...
        outputs = {
            ...
            'aux_outputs': []  # Could add auxiliary outputs for deep supervision
        }
    ...
    def training_step(self, batch, batch_idx):
        ...
        try:
            ...
            # Compute losses with original weights
    ...
    def prepare_targets(self, batch, max_points, valid_indices):
        """Prepare targets maintaining data integrity"""
    ...
    def compute_sem_loss(self, batch, sem_logits, valid_indices, padding):
        ...
        try:
            ...
            if all_logits:
                ...
                # Use original semantic loss weight
                sem_loss = F.cross_entropy(combined_logits, combined_labels, ignore_index=0)
                return sem_loss * self.cfg.LOSS.SEM.WEIGHTS[0]
            else:
                return torch.tensor(0.0, device='cuda', requires_grad=True)
    ...
    def panoptic_inference(self, outputs, padding):
        """Panoptic inference following original"""
    ...
    def export_to_onnx(self, epoch, pq, iou):
        ...
        try:
            ...
            onnx_dir = Path("experiments") / f"{self.cfg.EXPERIMENT.ID}_perf_v4" / "onnx_exports"
    ...
    def configure_optimizers(self):
        """Optimizer configuration matching original"""
        # Use original AdamW with proper weight decay
...
@click.option("--onnx_interval", type=int, default=5, help="Export ONNX model every N epochs")
def main(epochs, batch_size, lr, gpus, num_workers, nuscenes, checkpoint, onnx_interval):
    """Performance-optimized MaskPLS training with accuracy fixes"""
    
    print("="*60)
    print("Performance-Optimized MaskPLS Training v4")
    print("Fixing accuracy issues while maintaining speed")
    print("="*60)
    ...
    # Update config with original values for better accuracy
    ...
    # Use original point sampling values for accuracy
    dataset = cfg.MODEL.DATASET
    cfg[dataset].SUB_NUM_POINTS = 80000 if dataset == "KITTI" else 50000  # Original values
    
    # Use original loss parameters
    cfg.LOSS.NUM_POINTS = 70000  # Original value
    cfg.LOSS.NUM_MASK_PTS = 500  # Original value
    ...
    print(f"  ONNX Export: Every {onnx_interval} epochs")
    
    # Create data module
    data = SemanticDatasetModule(cfg)
    
    # Create model
    model = PerformanceOptimizedMaskPLS(cfg, onnx_interval=onnx_interval)
    ...
    tb_logger = pl_loggers.TensorBoardLogger(
        "experiments/" + cfg.EXPERIMENT.ID + "_perf_v4",
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="metrics/pq",
        filename=cfg.EXPERIMENT.ID + "_perf_epoch{epoch:02d}_pq{metrics/pq:.2f}",
